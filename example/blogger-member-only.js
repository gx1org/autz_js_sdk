...
<script>
  ...
  const subscribeFirstAlert = 'Akun Anda belum tercatat sebagai member. Silakan hubungi kami untuk berlangganan.'
  const autzorgAppID = 'qinw2is'
  ...
  async function main() {
    window.autz = new Autzorg(autzorgAppID)
    await autz.check()
    if (autz.error) {
    ...
    if (!window.SUBSCRIBER_IDs.includes(autz.user.id)) {
      postBody.innerHTML = `${subscribeFirstAlert} <a href="${subscribeLink}" target="_blank">Click here</a>`
      postBody.style.display = prevDisplay
      return
    }
    postBody.style.display = prevDisplay
